...
type SettingsSection = "java-runtime" | "appearance" | "ui-layout" | "language" | "about";
...
const sectionNav: { id: SettingsSection; label: string; icon: string }[] = [
  { id: "java-runtime", label: "Java 与运行", icon: "coffee" },
  { id: "appearance", label: "外观", icon: "palette" },
  { id: "ui-layout", label: "UI 布局", icon: "grid_view" },
  { id: "language", label: "语言", icon: "language" },
...
export default function SettingsScreen() {
  ...
  useEffect(() => { loadAllCustomBgPreviews(); }, []);
  ...
  useEffect(() => { applyBg(currentBg); }, [currentBg]);
  ...
  useEffect(() => { applyFont(selectedFont); }, [selectedFont]);

  function resetFont() { setSelectedFont(DEFAULT_FONT); }
  ...
  const freePercent = useMemo(() => Math.max(0, 100 - usedPercent), [usedPercent]);
    ...
    <>
      <nav className="settings-rail">
        {sectionNav.map((sec) => (
          <button
            key={sec.id}
            className={`settings-rail-item${activeSection === sec.id ? " active" : ""}`}
            onClick={() => setActiveSection(sec.id)}
          >
            <md-ripple />
            <span className="material-symbols-outlined settings-rail-icon">{sec.icon}</span>
            <span className="settings-rail-label">{sec.label}</span>
          </button>
        ))}
      </nav>

      <div className="settings-body">
        {activeSection === "java-runtime" && (
            ...
            <div className="expander-group">
              <div className="expander-header" onClick={() => setJavaExpanded(!javaExpanded)}>
                <div className="setting-card-info">
                    <label className="setting-label">Java 与运行</label>
                  <span className="setting-desc">检测到的 Java 安装</span>
                </div>
                <div className="expander-right">
                  <button className="refresh-btn" onClick={(e) => { e.stopPropagation(); fetchJavaVersions(); }} disabled={javaLoading}>
                    {javaLoading ? <span className="material-symbols-outlined spin" style={{fontSize:16}}>sync</span> : <span>刷新</span>}
                  </button>
                  <span className={`material-symbols-outlined expander-chevron${javaExpanded ? " open" : ""}`}>expand_more</span>
                </div>
              </div>
              {javaExpanded && (
                <div className="expander-body">
                  ...
                  {javaInstalls.map((jv, idx) => (
                    <div key={jv.path} className={`java-item${idx === javaInstalls.length - 1 ? " last" : ""}`}>
                      <div className="java-item-divider" />
                ...
                <div className="mem-item">
                  <div className="mem-slider">
                    <div className="mem-slider-bar">
                      <div className="mem-slider-segment mem-slider-used" style={{ width: usedPercent + "%" }} />
                      <div className="mem-slider-segment mem-slider-free" style={{ width: freePercent + "%" }} />
                    </div>
                  </div>
            ...
            <div className="setting-card col">
              ...
              <div className="bg-gallery">
                ...
                <div className={`bg-card${currentBg === "default1" ? " active" : ""}`} onClick={() => setCurrentBg("default1")}>
                  <div className="bg-preview"><img src={default1Bg} alt="" /></div>
                ...
                {customBgs.map(cbg => (
                  <div key={cbg.path} className={`bg-card${currentBg === "custom:" + cbg.path ? " active" : ""}`} onClick={() => setCurrentBg("custom:" + cbg.path)}>
                    <div className="bg-preview"><img src={customBgPreviews[cbg.path]} alt={cbg.name} /></div>
                    <span className="bg-name">{cbg.name}</span>
                  </div>
                ))}
                <div className="bg-card upload-card" onClick={uploadBg}>
                  <div className="bg-preview"><span className="material-symbols-outlined" style={{fontSize:24,fontVariationSettings:"'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24"}}>upload</span></div>
